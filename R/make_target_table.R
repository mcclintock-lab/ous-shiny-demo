...
progress <- bind_rows(list(rec_fishing, comm_fishing, tour_fishing, 
                           vessels, population, uw_cultural, rec_sports, 
                           sci_tech, aquaculture, sec_def))

# get rid of individual target dfs
...
breaks <- seq(0, 1, 0.01)
colors <- colorRampPalette(c("#FFD1C2", "#FCFACE", "#EAFCEC"))(length(breaks) + 1)
...
make_target_table <- function() {
  ...
  datatable(df, colnames = c("Metric", "Target", "Individual respondents",
